...
if (!function_exists('_form_common')) {
    ...
    function _form_common($type = 'text', $data = '', $value = '', $label = '', $extra = '', $tooltip = '') {
        ...
        //Extra Patch for Jquery handling

        $extra.=' id="' . $defaults['name'] . '" ';
        $output = _parse_form_attributes($data, $defaults);
        // Label Width set?
        $output = <<<EOL

<div class="form-group {$error}">
	<label class="control-label col-xs-2 " for="{$defaults['name']}" >{$label}</label>
	<div class="col-xs-10">
		 <input {$output} {$extra} />
		{$tooltip}
	</div>
...
if (!function_exists('form_dropdown')) {
    ...
    function form_dropdown($data, $options = array(), $selected = array(), $label = '', $extra = '', $tooltip = '') {
    	...
    	$classprefix=" col-xs-10 ";
		if (isset($extra["element_prefix_class"])){
			$classprefix=$extra["element_prefix_class"];
		}
		$element_class='';
		if (isset($extra['element_class'])){
			$element_class=' class= "'.$extra['element_class'].'"';
		}
...
<div  class="form-group {$error}">
	...
	<label class="control-label col-xs-2 " for="{$data['id']}">{$label}</label>
	<div class="{$classprefix}">
		 <select {$output} {$element_class}>
			{$options_vals}
		</select>
		{$tooltip}
	</div>
